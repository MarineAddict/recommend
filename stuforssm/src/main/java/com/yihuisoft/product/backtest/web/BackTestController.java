...
import com.yihuisoft.product.backtest.service.BackTestService;
...
public class BackTestController {
     ...
     * @Description:获取回归测试组合基本数据
     * @param planId @param cusId
     * @return  List<GroupInfo>
     * @date:   2018年02月03日
     */
    @RequestMapping("/getBacktestBasicData")
    @ResponseBody
    public List<GroupInfo> getBacktestBasicData(String groupCode,String groupType){


        List<GroupInfo> list=new ArrayList<>();

        list=backTestService.selectGroupBasicInfo(groupCode,groupType);

        //前端模拟测试数据
//        if("001".equals(groupCode)){
//            String[] productCode={"001.OF","000529.OF","000129.OF",
//                    "000729.OF","000829.OF","000929.OF",
//                    "000366.OF"};
//
//            String[] productName={"湖人001","凯尔特人","火箭",
//                    "勇士","骑士","马刺","热火"};
//
//            for(int i=0;i<productCode.length;i++){
//                GroupInfo groupInfo=new GroupInfo();
//                groupInfo.setProductCode(productCode[i]);
//                groupInfo.setProductName(productName[i]);
//                groupInfo.setWeights(1.0/productCode.length);
//                list.add(groupInfo);
//            }
//        }
//
//        if("002".equals(groupCode)){
//            String[] productCode={"002.OF","000529.OF","000129.OF",
//                    "000729.OF","000829.OF","000929.OF",
//                    "000366.OF"};
//
//            String[] productName={"湖人002","凯尔特人","火箭",
//                    "勇士","骑士","马刺","热火"};
//
//            for(int i=0;i<productCode.length;i++){
//                GroupInfo groupInfo=new GroupInfo();
...
//        }
    ...
    * @date:   2018年02月03日
    */
    @RequestMapping("/getBacktestMetricsData")
    @ResponseBody
    public List<GroupMetrics> getBacktestMetricsData(String groupType,String groupCode, String startTime, String endTime,String initmoney,String rebalance,String benchmark,String dateType){
...
//        //Table 01
//
        String[] assetIndexTitleArr={"Code","Worst Year","Stdev","Sortino Ratio","Sharpe Ratio","Max.Drawdown","Market Correlation","CAGR","Best Year"};
...
//        groupMetrics.setCodeNameList(codeNameList);
     ...
     * @Description: 自定义回测组合
     * @Date: 2018-03-02
     * @param  Groupname 产品组合名称
     *          assetsType 组合类型
     *          list  组合数据
     * @return
     */
    @RequestMapping(value = "/insertProductGroup/backTest",method = RequestMethod.POST)
    @ResponseBody
    public String insertProductGroup(String Groupname, String assetsType, ProductGroupDetails list){
        String groupId= "150";   //测试用
        try {
            Groupname=new String(Groupname.getBytes("ISO-8859-1"),"utf-8");
            groupId = backTestService.insertBacktestProductGroup(Groupname,assetsType,list);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    /**
     * @author yongquan.xiong
     * @Description: 根据产品类型查询产品代码
     * @Date: 2018-03-05
     * @param  productType 产品类型
     * @return
     */
    @RequestMapping(value = "/findProductCode",method = RequestMethod.POST)
    @ResponseBody
    public List<String> findProductCode(String productType){
        List<String> codeList=new ArrayList<>();
        try {
            codeList = backTestService.findProductCode(productType);
        } catch (Exception e) {
            e.printStackTrace();
            logger.info("查询产品代码失败!");
        }
        return codeList ;
    }
